...
def nanmean(x):
    ...
    Returns:
        torch.Tensor: Mean value with NaN values ignored
    """
...
def eval_metrics(true, pred, num_classes):
    """Compute segmentation metrics on 2D feature maps.

    Args:
        true (torch.Tensor): Ground truth labels of shape [B, H, W] or [B, 1, H, W]
        pred (torch.Tensor): Predicted logits of shape [B, num_classes, H, W]
        num_classes (int): Number of classes to segment

    Returns:
        tuple: (overall_acc, avg_per_class_acc, avg_jacc, avg_dice) - Segmentation metrics
    """
